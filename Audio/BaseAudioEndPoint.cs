...
public abstract class BaseAudioEndPoint : IAudioSource, IAudioSink
{
    ...
    //public bool MediaSessionReady
    //{
    //    get { return _isMediaSessionReady; }
    //    set { _isMediaSessionReady = value; }
    //}
    ...
    public void ExternalAudioSourceEncodedSample(uint durationRtpUnits, byte[] sample)
    {
        ...
        if (!isCanceled && isReady && hasEventHandler)
        {
            ...
            Log.Debug($"[{GetType().Name}] Forwarded encoded sample: {sample.Length} bytes, {durationRtpUnits} RTP units");
        }
        else
        {
